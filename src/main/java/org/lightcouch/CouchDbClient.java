...
public final class CouchDbClient extends CouchDbClientBase {
	...
	public CouchDbClient(String dbName, boolean createDbIfNotExist,
			String protocol, String host, int port, String username, String password, boolean revsInfo) {
		super(new CouchDbConfig(dbName, createDbIfNotExist, protocol, host, port, username, password, revsInfo));
	}
	...
	public <T> T find(Class<T> classType, String id) {
        ...
        if (config.isRevsInfo())
            builder.query("revs_info", "true");
	...
	public <T> T find(Class<T> classType, String id, String rev) {
        ...
        URIBuilder builder = builder(getDBUri()).path(id).query("rev", rev);
        if (config.isRevsInfo())
            builder.query("revs_info", "true");
		return get(builder.build(), classType);
	}
